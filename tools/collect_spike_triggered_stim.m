function [X, spikes, num_total_spikes] = collect_spike_triggered_stim(Stim, spike_train, n)

% input:
%       Stim = (time bin) x (space)
%       spike_train = (time bin) x (spikes)
%       n = number of samples for analysis
% Output:
%       X = (num_spikes) x (time bin x dim)
...
% "spike vector" including # of spikes (>0) for each bin
