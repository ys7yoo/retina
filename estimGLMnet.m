...
for n = channel_index_to_analyze
    ...
    % [Another] ROI mask based on the center of RF and radius
...
%% DEBUG for different mask sizes
% UU = u(:,end)
% save mask_5x5 UU sta_num_samples XX YY mask width height
% save mask_3x3 UU sta_num_samples XX YY mask width height


if 1==0
    %% compare filters of two masks
    mask_3x3 = load('mask_3x3.mat')
    mask_5x5 = load('mask_5x5.mat')    
    ...
    subplot(211)
    hist(mask_3x3.UU, bins)
    xlabel('eigenvector with the smallest eigen value')
    %title('histogram of eigenvector with the smallest eigen value')
    ylabel('count')
    box off
    title('ROI 3x3')
    
    subplot(212)
    hist(-mask_5x5.UU, bins)
    xlabel('-eigenvector with the smallest eigen value')
    ylabel('count')
    box off
    %title('histogram of eigenvector with the smallest eigen value (sign flipped)')
    title('ROI 5x5')
end
